...
static mut RFFT_ARRAY_A: [u16; crate::SAMPLE_LEN] = [0; crate::SAMPLE_LEN];
static mut RFFT_ARRAY_B: [u16; crate::SAMPLE_LEN] = [0; crate::SAMPLE_LEN];

static mut RFFT_INDEX: usize = 0;
...
pub fn adc_to_rfft() {
    ...
    // 获得数组的索引
    let mut rfft_index = unsafe { RFFT_INDEX };

    // 看不懂了，有更好的写法欢迎告诉我
    const SAMPLE_LEN0: usize = SAMPLE_LEN - 1;
    const SAMPLE_LEN1: usize = SAMPLE_LEN + 1;
    const SAMPLE_LEN2: usize = SAMPLE_LEN * 2 - 1;
    let static_arrary = unsafe {
        match rfft_index {
            0..=SAMPLE_LEN0 => addr_of_mut!(RFFT_ARRAY_A),
            SAMPLE_LEN => {
                critical_section::with(|cs| {
                    AB_FLAG.borrow(cs).set(true);
                });
                addr_of_mut!(RFFT_ARRAY_B)
            }
            SAMPLE_LEN1..=SAMPLE_LEN2 => addr_of_mut!(RFFT_ARRAY_B),
            _ => {
                rfft_index = 0;
                critical_section::with(|cs| {
                    AB_FLAG.borrow(cs).set(false);
                });
                addr_of_mut!(RFFT_ARRAY_A)
            }
        }
    };
    ...
    unsafe {
        let index = match rfft_index >= SAMPLE_LEN {
            true => rfft_index - SAMPLE_LEN,
            false => rfft_index,
        };
        (*static_arrary)[index] = pin_mv;
        RFFT_INDEX = rfft_index + 1;
    }
...
pub async fn rfft(gain_index: usize) -> f32 {
    ...
    critical_section::with(|cs| {
        rfft_array = match AB_FLAG.borrow(cs).get() {
            true => unsafe { RFFT_ARRAY_A },
            false => unsafe { RFFT_ARRAY_B },
        }
        .clone()
        .map(|val| val as f32);
    });
